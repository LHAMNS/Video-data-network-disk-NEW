...
# 从converter模块导入需要的类和函数
from converter import (
    CacheManager, ReedSolomonEncoder, FrameGenerator,
    VideoEncoder, StreamingVideoEncoder, calculate_video_params
)
# 从frame_generator模块导入OptimizedFrameGenerator
from converter.frame_generator import OptimizedFrameGenerator
...
class ConversionTask:
    ...
    def _frame_generated_callback(self, frame_idx, total_frames, frame):
        ...
        if (current_time - self.last_progress_update >= update_interval or 
            frame_idx == 0 or frame_idx >= total_frames - 1):
    ...
    def _verify_output_video(self):
        ...
        try:
            ...
            if fix_result.returncode == 0 and temp_output.exists() and temp_output.stat().st_size > 0:
                ...
                # 修复失败但继续尝试验证原文件
                logger.warning(f"MP4文件修复失败，继续尝试验证原文件: {self.output_path}")
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    if "streams" in output and len(output["streams"]) > 0:
                        logger.info(f"视频文件验证成功: {self.output_path}, 信息: {output['streams'][0]}")
                        return True, "视频有效"
                    else:
                        return False, "无法获取视频流信息"
                except Exception as e:
                    return False, f"解析视频信息失败: {e}"
            else:
                error_msg = result.stderr.decode('utf-8', errors='ignore')
                logger.error(f"视频文件验证失败: {error_msg}")
                return False, f"视频文件验证失败: {error_msg}"
