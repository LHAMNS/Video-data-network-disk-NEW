...
def verify_video_file(video_path):
    ...
    try:
        ...
        info_result = subprocess.run(info_cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        ...
        if info_result.returncode != 0:
            ...
            try:
                ...
                logger.info(f"视频信息: {json.dumps(video_info, indent=2)}")
            except:
                logger.warning("视频信息解析失败")
        
        return True, "视频文件有效"
        
    except Exception as e:
        logger.error(f"验证视频时出错: {e}", exc_info=True)
